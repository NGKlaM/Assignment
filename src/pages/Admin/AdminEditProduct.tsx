...
const AdminEditProduct = () => {
  ...
  const [updateProduct, setProductUpdate] = useState<ProductFormParams>({
    title: "",
    category: "",
    image: "",
    price: 0,
  ...
  const handleChangeForm = (
    ...
    setProductUpdate({ ...updateProduct, [event.target.name]: event.target.value });
  };
  ...
  const handleSubmitForm = async (event: React.SyntheticEvent) => {
    ...
    if (!productId) return;
    try {
      const { data } = await axios.put(`/api/products/${productId}`, updateProduct);
      toast.success("Update Product Successfull!", data.data);
